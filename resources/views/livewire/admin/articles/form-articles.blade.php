<div class="p-4 md:p-5 space-y-4">
    <form wire:submit.prevent="{{ $actionForm }}">
        ...
        @if($editForm == true)
            <input type="hidden" wire:model="idArticle" id="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="@lang('Title')">
        @endif
        ...
        <div class="grid gap-4 md:grid-cols-4 sm:gap-6">
            ...
            <div class="col-span-1">
                <div class="flex flex-col pb-8">
                    ...
                    <a href="{{ route('app-articles') }}" class="w-full text-center block text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">@lang('Cancel')</a>

                </div>
                <div class="text-right border-t-4 border-indigo-500 pt-4">
                    <label class="relative inline-flex items-center mb-5 cursor-pointer">
                        ...
                        <span class="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">@lang('Status article')</span>
                    </label>
                </div>
                <div class="border-t-4 border-indigo-500 pt-4">

                    {{-- dd(json_encode($categoriesArticle)) --}}
                    {{-- dd(count(json_decode(json_encode($categoriesArticle)))) --}}
                    <fieldset>
                        <label for="slug" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">@lang('Categories')</label>
                        {{-- dd($categories) --}}

                        @forelse($categories as $categorie)
                            <div class="flex items-center mb-4" wire:ignore.self>
                                <input id="checkbox-{{ $categorie->id_post_category }}" type="checkbox" value="{{ $categorie->id_post_category }}" wire:model="categoriesArticle" @if(in_array($categorie->id_post_category, $categoriesArticle)) checked @endif class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                <label for="checkbox-{{ $categorie->id_post_category }}" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $categorie->name_category }}</label>
                            </div>
                        @empty
                ...
                <div class="border-t-4 border-indigo-500 pt-4">
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="imageProfile">@lang('Image article')</label>
                    <div class="">
                        <div class="">
                            @if($seoImage)
